...
int dotsEaten=0;
typedef struct // structure Queue to impelement BFS alg
{
	int arr[1000]; 
	int i,j; 
} Queue;
// Codeby FarnoodID
void enqueue(Queue *q,int a) //enqueue for BFS alg
{
	q->arr[q->j++] = a;
}
int isEmpty(Queue *q) //Check if empty for BFS alg
{
	return q->i >=  q->j;
}
int dequeue(Queue *q) //dequeue for BFS alg
{
	return q->arr[q->i++];
}
...
int oneMove(char map[100][100],int position,int column,int row,int dotsPosition[100],int dotsAtFirst)			//computer choosing a side in which to get to the nearest dot using BFS alg
{
...
void border (char map[100][100],int column,int row)														//making borders & printing map
{
	...
	{
		// This is for coloring the border blue
		printf("\033[1;34m");
		putchar(178);
		// This is for refreshing the color of printing back to normal
		printf("\033[0m");
	}
	putchar('\n');
	for(int i=0;i<row;i++)
...
int main ()
{
	while(1)
	{
		...
		int testcaseNum=1,row=0,column=0,checker=0,checkBlank=0,position=0,sizeOfColumn=0,side=1,dots=0,lifes=3,ch=0,ch2=0,autoModeOn=0,dotsAtFirst=0,dotsPosition[100]={0},indexForDots=0,pDot=0;
